				...
				if (pcmd)
				{
					...
					if (strcmp(pcmd->valuestring, "login") == 0)
					{
						...
						if(iRetCode<0)
						{
							...
							int iSessionid = -1;
							if(pSionid) iSessionid = atoi(pRetCOde->valuestring);
							if(Istype == 1)
							{
								...
								if(istused != this0->m_pManager->m_SetUsedRegionID.end())
								{
									...
									printf("-----123erase region %s success \n",txt);
									fflush(stdout);
					...
					{
						printf("--check query_type return \n");
						fflush(stdout);
						//通过
  ...
  if(Istype != 2)
  {
	...
	//与enrich对接
		if(Istype == 0)
		{
			...
			char ipqamport[64] = {0};
			MapRegionID::iterator pit = m_pManager->m_mapRegionID.begin();
			while(pit!=m_pManager->m_mapRegionID.end())
			{
				if (strcmp(strRegionid,pit->second)== 0)
				{
					iport = pit->first;
					printf("----region = %s,port = %d\n",pit->second,pit->first);
					fflush(stdout);
					sprintf(ipqamport,"%d",iport);
					break;
				}
				++pit;
			}
    ...
    if(Istype == 2)
    {
		...
		if(p)
		{
			*p = 0;
			...
			if(advflag == 0)
			{
				sprintf(output,"udp://%s:%d",strAdverIP,65000+iStreamID);
				printf("totalTime = %s,posturl = %s,outputurl = %s\n",totalTime,strUrl,output);
				fflush(stdout);
			}
			else if(advflag == 1)
			{
