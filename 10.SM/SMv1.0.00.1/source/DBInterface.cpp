...
bool DBInterfacer::LoadALLIPQAMRes(int ipqaminid,int *port)
{
#ifdef USEMUTEX
		pthread_mutex_lock(&locker);
#endif
		...
		pthread_mutex_unlock(&locker);
#endif	
		UpdateIPQAMRes(ipqamID);
		LOG_DEBUG_FORMAT("DEBUG  - [SM]:%s %d IPQAM resource return\n",__FUNCTION__,__LINE__);
		return true;
	
}

bool DBInterfacer::UpdateIPQAMRes(int myid)
{
#ifdef USEMUTEX	
	pthread_mutex_lock(&locker);
#endif	
	...
	sprintf(buf,"update ipqam_resource set state = 1 where ipqamResId = %d",myid); //,tab1->strSessionID,tab1->strSerialNo
	LOG_INFO_FORMAT("INFO  - [SM]:%s %d %s\n",__FUNCTION__,__LINE__,buf);
	if(mysql_ping(conn_ptr)!=0)
	{
		LOG_ERROR_FORMAT("ERROR  - [SM]:%s %d ping error: -1004\n",__FUNCTION__,__LINE__);
...
bool DBInterfacer::ResetIPQAMRes()
{
	...
	sprintf(buf,"update ipqam_resource set state = 0 "); //,tab1->strSessionID,tab1->strSerialNo
	LOG_INFO_FORMAT("INFO  - [SM]:%s %d %s\n",__FUNCTION__,__LINE__,buf);
	if(mysql_ping(conn_ptr)!=0)
	{
		LOG_ERROR_FORMAT("ERROR  - [SM]:%s %d ping error: -1004\n",__FUNCTION__,__LINE__);
