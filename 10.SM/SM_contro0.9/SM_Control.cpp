			...
			if (pRoot)
			{
				...
				if (pcmd)
				{
					...
					else if(strcmp(pcmd->valuestring, "vod_play") == 0)
					{
						...
						fflush(stdout);
						//调manager 中点播
						 ret = this0->m_cSM_Manager->AddOneVodStream(iStreamID,strUrlAddr,accept_fd,vodname,posterurl,strSerialNo);//需要参数
						if(ret>=0)
						{
			...
			*/		/*	cJSON *pRet_root;
				...
				else if(strcmp(pcmd->valuestring,"quit")==0)
				{
				...
				else if(strcmp(pcmd->valuestring,"recovery")==0)
				 {
					...
					if(pStreamID->valuestring)
					{
						printf("----this is Recovery streamid = %s",pStreamID->valuestring);
						fflush(stdout);
						if(pStreamID) iStreamID = atoi(pStreamID->valuestring);
					}
					//username
					...
					if(pUsername)
					{
						memcpy(iusername,pStreamID->valuestring,strlen(pStreamID->valuestring)+1);
					}	
