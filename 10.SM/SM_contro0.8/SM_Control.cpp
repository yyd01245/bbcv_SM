				...
				if (pcmd)
				{
					...
					//	printf("---request parse \n");
						...
						*/
						...
						printf("9999999csip = %s\n",csip);
						printf("999999csport = %s\n",csport);
			...
			*/		/*	cJSON *pRet_root;
					...
					*/
					...
					else if(strcmp(pcmd->valuestring,"vod_pause")==0)
					{
						...
						if(pStreamID->valuestring!=NULL)
						{
							...
							this0->m_cSM_Manager->PauseVOD(iStreamID,iUsername);
						}
						Stream ptmpRequest;
				  ...
				  else if(strcmp(pcmd->valuestring,"vod_resume")==0)
				  {
						...
						{	
							...
							if(pStreamID) iStreamID = atoi(pStreamID->valuestring);
							this0->m_cSM_Manager->RecoverVodPlay(iStreamID);
						}
						Stream ptmpRequest;
...
{
	printf("11111111111111111111111111hello\n");
	SM_Control *this0 = (SM_Control*)arg;
			...
			if(pRoot)
			{
				...
				if(strcmp(pcmd->valuestring,"user_action")==0)
				{
					...
					if(pStreamID)
					{
						...
						{
							iStreamID = 0;
							printf("7777777777777777777777777777777ok\n");
						}
						else
						iStreamID = atoi(pStreamID->valuestring);
					}
