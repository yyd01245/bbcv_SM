...
//	usleep(1000);
	...
	while(iterGroup != m_mapStreamGroup.end())
	{
		...
		//·Ö±ð¼ÓÔØ¸ß±êÇåµÄÁ÷
			...
			DBInterfacer::GetInstance()->FindSameGroupStream(ptmp->strNetRegionNum,0,sdlistTmps);
			DBInterfacer::GetInstance()->FindSameGroupStream(ptmp->strNetRegionNum,1,hdlistTmph);
