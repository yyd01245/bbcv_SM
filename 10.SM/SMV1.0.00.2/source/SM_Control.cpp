...
//	pthread_detach(tcp_recv_thread3);
	...
	pthread_create(&tcp_recv_thread6, NULL, ts_recv_Clear_thread, this);
...
void *SM_Control::ts_recv_Nav_thread(void *arg)
{
	...
	{
		LOG_ERROR_FORMAT("ERROR  - [SM]:%s %d create socket\n",__FUNCTION__,__LINE__);
		return NULL;
	} 
	...
	while(1)
	{
			...
			if (pRoot)
			{
				...
				if (pcmd)
				{
					...
					{
						LOG_DEBUG_FORMAT("DEBUG  - [SM]:%s %d add_ads_stream return\n",__FUNCTION__,__LINE__);
						//通过
