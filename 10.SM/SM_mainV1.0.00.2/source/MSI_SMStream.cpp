...
bool MSI_SM_Stream::Connect_MSIServer()
{
	...
	LOG_INFO_FORMAT("INFO  - [SM]:%s %d BEGIN TO CONNECT THE MSISECER\n",__FUNCTION__,__LINE__);
...
void *MSI_SM_Stream::Parse_recv_MSI_thread(void * arg)
{
	...
	int iRecvLen = 0;
	int accept_fd = 0;
	Sockfd::iterator s_iit = this0->connsocket.begin();
	if(s_iit != this0->connsocket.end())
	{
		accept_fd = *s_iit;
		this0->connsocket.erase(s_iit);
	}
	while(1)
	{
