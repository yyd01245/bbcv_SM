...
public:
	...
	int AddOneNavStream(int iStreamID,int streamtype);
	...
	bool Navgatinsert(int flagid);
	bool Strrouseinsert(int streamid);
	bool Navgatdelete(int flagid);
	bool Strrousedelete(int streamid,char *strUsername);
...
private:
	...
	Navnext hdm_navnext;   //用于同一区域下装载即将下发的高清导航流的标志
	Navnext sdm_navnext;   //用于同一区域下装载即将下发的标清导航流的标志
	FailNav hd_failnav;   //用于存放同一区域下未下发成功的高清导航流的个数
	FailNav sd_failnav;   //用于存放同一区域下未下发成功的标清导航流的个数
	StreamAbnormal m_allstrem; //用于存放当前所有流id以及高标清标志
	RegNavurl m_Navurl; //用于记录各个区域下导航流地址被重用的情况
};
