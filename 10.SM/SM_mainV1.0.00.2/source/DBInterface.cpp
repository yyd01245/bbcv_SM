...
bool DBInterfacer::LoadAlladvinfo(SetAdvinfo &mapAdvinfo,int temp)
{
	int res;
	char strSQL[1024] ={0};
	if(temp == 0)
		...
		sprintf(strSQL,"select * from ucs_adv_info where id = %d",temp);
...
bool DBInterfacer::FindSameNavflag(int strtype,Setstrnavflag& setStreamTmp)
{
#ifdef USEMUTEX
	pthread_mutex_lock(&locker);
#endif
	int res;
	char strSQL[1024] ={0};
	sprintf(strSQL,"select * from ucs_navigate where resolution = %d",strtype);
	LOG_INFO_FORMAT("INFO  - [SM]:%s %d %s\n",__FUNCTION__,__LINE__,strSQL);
	...
	if(!res)
	{
		...
		{			
			...
			{				
				...
				if(sqlrow[0] != NULL)
				{
					flagID = atoi(sqlrow[0]);
					setStreamTmp.insert(flagID);
				}
			}	
			mysql_free_result(res_ptr);
		}
...
bool DBInterfacer::FindNavurl(int flagid,char *naurl)
{
 #ifdef USEMUTEX
	pthread_mutex_lock(&locker);
 #endif
	int res;
	char strSQL[1024] ={0};
	sprintf(strSQL,"select * from ucs_navigate where id = %d",flagid);
	LOG_INFO_FORMAT("INFO  - [SM]:%s %d %s\n",__FUNCTION__,__LINE__,strSQL);
	...
	if(!res)
	{
		...
		{			
			...
			{				
				if(sqlrow[2] != NULL)
				{
					memcpy(naurl,sqlrow[2],strlen(sqlrow[2])+1);
					LOG_DEBUG_FORMAT("DEBUG  - [SM]:%s %d flagid = %d,navurl = %s\n",__FUNCTION__,__LINE__,flagid,naurl);
				}
			}	
			mysql_free_result(res_ptr);
		}
...
bool DBInterfacer::Findtype(int streamid,int type,int flag)
{
 #ifdef USEMUTEX
	pthread_mutex_lock(&locker);
 #endif
	...
	char strSQL[1024] ={0};
	if(flag == 0)
		sprintf(strSQL,"select *from ucs_stream_resource where iStreamID = %d",streamid);
	else
		sprintf(strSQL,"select *from ucs_navigate where id = %d",streamid);
	...
	if(!res)
	{
		...
		{			
			...
			{				
				if(flag == 0)
				{
					//表ucs_stream_resource
					if(sqlrow[5] != NULL)
					{
						type = atoi(sqlrow[5]);
						LOG_DEBUG_FORMAT("DEBUG  - [SM]:%s %d streamid = %d,type = %s\n",__FUNCTION__,__LINE__,streamid,type);
					}
				}
				else
				{
...
bool DBInterfacer::FindlNavid(int strtype,int id)
{
#ifdef USEMUTEX
	pthread_mutex_lock(&locker);
#endif
