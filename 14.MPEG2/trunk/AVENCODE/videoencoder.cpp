...
char Mpeg2SequenceHead[1024]={0};
int iMpeg2SequenceHeadLen = 0;

int Find_MPEG2_GropHead(unsigned char* buff,int ilen)
{
	int iPos = -1;
	unsigned char *pFrameBuff = buff;
	int iCmpLen = 0;
	while(iCmpLen < ilen-4 && iCmpLen < 512)
	{
		if(*pFrameBuff == 0x00 && *(pFrameBuff+1)==0x00 
			&& *(pFrameBuff+2)==0x01 && *(pFrameBuff+3)==0xb8)
		{
...
*/	
		...
		if(end_time_us - start_time_us > 25)
			fprintf(stderr,"======encoder time %ld \n",end_time_us - start_time_us);
	return 0;
}
