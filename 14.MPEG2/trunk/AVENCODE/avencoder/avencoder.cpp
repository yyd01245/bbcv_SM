...
#define version_str "MP2_1.0.00.1"
...
int main(int argc,char **argv)
{
	...
	const char *short_options = "i:w:h:e:b:d:p:s:g:f:l:?";
	...
	if(!has_encoderate)
		encode_bitrate = packet_bitrate*80/100;
	
	//TODO 检查各个参数的有效性
	printf("avencoder version %s\n",version_str);
	printf("++++++index=%d|width=%d|height=%d|smooth_bitrate=%d|packet_bitrate=%d|encode_bitrate=%d|destip=%s|destport=%d++++++\n",m_index,width,height,smooth_bitrate,packet_bitrate,encode_bitrate,destip,destport);
...
int write_video_frames()
{
	...
	*/
	...
	ret = encode_video_sample(p_video_encoder,p_video_source_sample,n_video_source_sample,p_video_encode_sample,&n_video_encode_sample);
...
//	if(NULL == fp)
//		fp = fopen("mpeg2","wb+");
//	fwrite(p_video_encode_sample,1,n_video_encode_sample,fp);
