...
public class DefaultSequenceGenerator implements SequenceGenerator {
	...
	protected PropertiesHelper ph = PropertiesFactory.getPropertiesHelper(PropertiesFile.APP);
	protected String ip = ph.getValue("urm.redisserver.ip");
	protected int port = Integer.parseInt(ph.getValue("urm.redisserver.port"));
	private String urm_redis_passwd = ph.getValue("urm.redisserver.password");
	private MDBInterface mdb = (MDBInterface) SpringBeanLoader.getSpringBean("urmmdbImpl");
	
	//要加载到缓存的键值
	protected String URMIDS = "NETWORKID,RFCODEID,URMIPQAMID,CODEID,";
	...
	public void init() {
		...
		boolean redisUsable = usable.jedisUseable(ip, port, urm_redis_passwd) ;
		try {
			String svalue = null ; 
			if(!redisUsable){
				logger.info("【错误】:URM缓存密码配置错误！");
			}else{
				svalue = mdb.getString(this.getId()) ;
			}
			//数据库值
			Long initValue1 = sequenceStorer.load(this.getId());
			
			Long initValue = initValue1 ;
			if(svalue!=null){
				if(Long.valueOf(svalue) > initValue1){
					initValue = Long.valueOf(svalue) ;
				}
			}
			...
			if(URMIDS.indexOf(getId()+",")!=-1 && redisUsable){
//				mdb.set(getId(), String.valueOf(maxCount));
				mdb.putString(getId(), String.valueOf(maxCount)) ;
			}
		} catch (Exception e) {
	...
	public long next() throws CreateSequnceException {		
		...
		boolean redisUsable = usable.jedisUseable(ip, port, urm_redis_passwd) ;
		try {
//			Jedis mdb = new Jedis(ip,port);
//			if(urm_redis_passwd!=null&&!"".equals(urm_redis_passwd)){
//				mdb.auth(urm_redis_passwd);
//			}
//		    String svalue = mdb.get(this.getId()) ;
		    ...
		    Long curr = null ;
		    if(svalue != null){
		    	curr = Long.valueOf(svalue);
		    }
			 
			if(curr!=null && curr>maxCount){
				maxCount = curr ;
				currCount = curr ;
			}
			
			if (currCount == maxCount){// 重新申请id
				long tmp = currCount + cache;
				if (tmp >= maxValue) {
					if (this.cycle) {
						tmp = this.minValue;
					} else {
			...
			if(URMIDS.indexOf(getId()+",")!=-1 && redisUsable){
//				mdb.set(getId(), String.valueOf(maxCount));	
				mdb.putString(this.getId(), String.valueOf(maxCount)) ;
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
