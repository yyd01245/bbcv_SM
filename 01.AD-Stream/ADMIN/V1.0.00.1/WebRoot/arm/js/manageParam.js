...
Ext.onReady(function() {
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span style="font-weight:bold;">全局参数表</span>',
				iconCls : 'configIcon',
				height : 500,
				// width:600,
				autoScroll : true,
	...
	function saveParamItem() {
		...
		addParamFormPanel.form.submit({
					...
					failure : function(form, action) {
						...
						Ext.MessageBox.show({title:'提示', msg:'参数数据保存失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
					...
					if (btn == 'yes') {
						...
						Ext.Ajax.request({
									...
									success : function(response) {
										...
										Ext.MessageBox.show({title:'提示', msg:'内存同步成功!' ,buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
				
									},
									failure : function(response) {
										Ext.MessageBox.show({title:'提示', msg:'内存同步失败!',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
	...
	function deleteParamItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.MessageBox.show({title:'提示', msg:'请先选中要删除的项目!' ,buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
					...
					if (btn == 'yes') {
						if (runMode == '0') {
							Ext.MessageBox.show({title:'提示', msg:
									'系统正处于演示模式下运行,您的操作被取消!该模式下只能进行查询操作!' ,buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
							return;
						}
						showWaitMsg();
						Ext.Ajax.request({
									...
									failure : function(response) {
										var resultArray = Ext.util.JSON
												.decode(response.responseText);
										Ext.MessageBox.show({title:'提示', msg:resultArray.msg ,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.MessageBox.show({title:'提示', msg:'请先选中要修改的项目!' ,buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
	...
	function update() {
		addParamFormPanel.form.submit({
					...
					failure : function(form, action) {
						var msg = action.result.msg;
						Ext.MessageBox.show({title:'提示', msg: '数据修改失败:<br>' + msg ,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
});