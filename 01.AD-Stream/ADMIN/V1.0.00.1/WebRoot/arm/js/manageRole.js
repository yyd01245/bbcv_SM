...
Ext.onReady(function() {
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span style="font-weight:bold;">角色信息</span>',
	...
	function addInit() {
		...
		addRoleWindow.show();
		addRoleWindow.setTitle('<span>新增角色</span>');
	...
	function saveRoleItem() {
		...
		addRoleFormPanel.form.submit({
					...
					failure : function(form, action) {
						...
						Ext.MessageBox.show({title:'提示',msg:'角色数据保存失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.MessageBox.show({title:'提示',msg:'请先选择要修改的角色!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		if (record.get('roletype') == '3') {
			Ext.MessageBox.show({title:'提示',msg:'系统内置角色,不能修改!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		addRoleFormPanel.getForm().loadRecord(record);
		addRoleWindow.show();
		addRoleWindow.setTitle('<span>修改角色</span>');
	...
	function update() {
		addRoleFormPanel.form.submit({
					...
					failure : function(form, action) {
						var msg = action.result.msg;
						Ext.MessageBox.show({title:'提示',msg:'角色数据修改失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
