...
public class StringUtil {
	...
	public static String getTodayDate() {
		...
		today += (now.getYear() + 1900) + "年" + (now.getMonth() + 1) + "月"
				+ now.getDate() + "日" + "   " + "星期" + weektemp;
		return today;
	}

	/**
	 * 格式化日期 created by wxj on May 29,2001
	 * 
	 * @Param strDate 待格式化字符串(yyyyMMddHHmmss) (MM表示月份)
	 * @Param strFormat 格式 (可用格式为yyyy yy mm dd hh HH mm ss) return value: if
	 *        strDate is null, return "", 若无法格式化，return strDate
	 */
	...
	//
	// //取路径分隔符"/" or "\"
	// public static String getPathSplit(javax.servlet.ServletContext
	// application ){
	// String strWebDir=application.getRealPath("/");
	 ...
	 * @return String 对象,如果成功返回处理后的sql； 或者返回原sql 如果没有成功
	 */
	public static String sqlAppendInsert(String sql, String appendColumn,
			String appendValue) {
		if (sql != null && !sql.equals("")) {
	 ...
	 */
	public static String sqlAppendInsert(String sql, String appendColumn,
			String appendValue, String columnType) {
		if (sql != null && !sql.equals("")) {
			int values = sql.indexOf(") values(");
			int begin = sql.indexOf("(");
			String sql1, sql2;
	 ...
	 * @return String 对象,如果成功返回处理后的sql； 或者返回原sql 如果没有成功
	 */
	public static String sqlAppendUpdate(String sql, String appendColumn,
			String appendValue) {
		if (sql != null && !sql.equals("")) {
			int where = sql.indexOf("where ");
				...
				if ((where - begin) > 5) {// 若sql中已有字段，则先加","后加字段
					...
					if (columnType.equals("date")) {// 若为日期型字段
						if (appendValue.equals("")) {// 无值，默认插入sysdate
							...
							sql1 = sql1 + "," + appendColumn + " =to_date('"
									+ appendValue
									+ "','yyyy-MM-dd hh24:mi:ss')";
						}
					} else {
