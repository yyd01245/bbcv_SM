    ...
    else
    {
        ...
        [SMS_SDK commitVerifyCode:self.verifyCodeField.text result:^(enum SMS_ResponseState state) {
            if (1==state) {
                ...
                JCO.smsCode = self.verifyCodeField.text;
                NSLog(@"block 验证成功");
...
//        UINavigationController* nav=[[UINavigationController alloc] initWithRootViewController:[[YJViewController alloc] init]];
        //JRegisterViewController *registerViewController=[[JRegisterViewController alloc] init];
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:SMS_SUCCESS_NOTIFICATION_NAME object:nil];
        }];
...
- (void)viewDidLoad
{
    ...
    NSString *icon = [NSString stringWithFormat:@"bg_summit"];
...
-(void)updateTime
{
    ...
    //NSLog(@"更新时间");
    self.timeLabel.text=[NSString stringWithFormat:@"%@%i%@",@"接受短信大约需要",60-count,@"秒"];
}
