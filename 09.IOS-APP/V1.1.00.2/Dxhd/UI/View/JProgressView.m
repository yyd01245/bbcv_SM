...
- (id)initWithFrame:(CGRect)frame withProgress:(float)progress
{
    ...
    if (self) {
        ...
        */
        ...
        if (!_endLabel) {
            self.endLabel = [[UILabel alloc] initWithFrame:CGRectMake(frame.size.width-44., yOffset, 52., size.height)];
        ...
        if (!_progressLabel) {
            self.progressLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 52., size.height)];
...
- (void)setProgressByCurrentTime:(float)currentTime totleTime:(float)totleTime
{
    if (totleTime != 0) {
        ...
        float progress = currentTime/totleTime;J_LOG(@"progress = %f", progress);
        ...
        //移动游标坐标
        self.progressLabel.center = CGPointMake((self.frame.size.width-PROGRESS_WDITH)/2+PROGRESS_WDITH*progress, self.progressLabel.frame.size.height/2-6.);
        //self.progressLabel.center = CGPointMake((self.frame.size.width-PROGRESS_WDITH)/2+PROGRESS_WDITH*progress, self.progressLabel.frame.size.height/2-2);
...
- (void)showProgressViewAndHideAfterDuring:(float)during;
{
    ...
    [self performSelector:@selector(hideFromSuperview) withObject:nil afterDelay:during];
}
