...
- (void)dealloc
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:PLAY_STATE_OPEN_NOTIFICATION_NAME object:nil];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:PLAY_STATE_CLOSE_NOTIFICATION_NAME object:nil];
}
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playOpen) name:PLAY_STATE_OPEN_NOTIFICATION_NAME object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playClose) name:PLAY_STATE_CLOSE_NOTIFICATION_NAME object:nil];
    }
    return self;
}
...
- (void)setCenterState:(CenterState)centerState
{
    ...
    J_LOG(@"_centerState = %d", _centerState);
...
- (void)loadKeyModel:(JKeyModel *)model
{
    if (model && model.return_code == 0) {
        ...
        if ([self.currentKeyValue isEqualToString:BlcIrrPropertyValue_EXIT]) {
            ...
            if (JCO.playChannel) {
                J_LOG(@">>>开始、暂停");
                if ([self.pauseButton.currentImage isEqual:[UIImage imageNamed:(@"bt_remoter_start")]]) {
                    [self setCenterState:PauseState];
                }
                else {
                    [self setCenterState:StartState];
            ...
            if ([model.totalTime floatValue] != 0) {
                ...
                if (para == 1) {//播放完
                    //查询
                    //[self sendQueryRequest];
                    if ([model.status isEqualToString:@"3"]) {J_LOG(@"播放完");
                        JCO.playChannel = NO;
                        [self performSelector:@selector(updatePlayState) withObject:nil];
                    }
                }
