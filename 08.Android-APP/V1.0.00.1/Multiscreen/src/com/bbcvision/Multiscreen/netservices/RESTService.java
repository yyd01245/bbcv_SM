...
public class RESTService {
		...
		http.send(HttpRequest.HttpMethod.POST, path, params,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						Log.i("userLoginInfo###:", responseInfo.result);
	...
	public void bindUesr(UserLoginInfo userLoginInfo, String stream_id,String url,
		...
		/*params.addBodyParameter("url", userLoginInfo.getUrl());
		params.addBodyParameter("vod_page", url);*/
		
		
		/*Log.i("bindUesrparams", userLoginInfo.getUsername()
				+userLoginInfo.getNew_token()
				+userLoginInfo.getSequence()+"stream_id"
				+stream_id+"url"
	...
	public void vodPlay(UserLoginInfo userLoginInfo, String url,String vodname,String posterurl,
		...
		Log.i("vodPlayParams", userLoginInfo.getUsername()+userLoginInfo.getNew_token()+userLoginInfo.getSequence()+url+vodname+posterurl);
	...
	public void chooseTime(UserLoginInfo userLoginInfo,String begintime,
		...
		//  userLoginInfo.getStream_id()重启后值为空！
		//Log.i("getStream_id", userLoginInfo.getStream_id());
		...
		http.send(HttpRequest.HttpMethod.POST, path, params,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						Log.i("sessionQuery", responseInfo.result);
    ...
    public static void getEweimaInfo(String scanMsg, final Callbackeweima callbackeweima) {

        String path = "http://218.108.50.246/wechat/oneway/urlmapqr?url="+scanMsg;
        HttpUtils http = new HttpUtils();
        http.send(HttpRequest.HttpMethod.GET, path,
                new RequestCallBack<String>() {
                    @Override
                    public void onStart() {
                        // UIUtils.showToastSafe("reging");
                    ...
                    @Override
                    public void onSuccess(ResponseInfo<String> responseInfo) {
                        ErweimaInfo erweimaInfo = null;
                        try {
                            erweimaInfo = (ErweimaInfo) JSONService.jsonToBean(responseInfo.result, ErweimaInfo.class);
                            callbackeweima.callbackEweima(erweimaInfo);
                        } catch (Exception e) {
                            erweimaInfo = new ErweimaInfo();
                            erweimaInfo.setRetcode("-2");
                            callbackeweima.callbackEweima(erweimaInfo);
                        }
                    ...
                    @Override
                    public void onFailure(HttpException error, String msg) {
                        ErweimaInfo erweimaInfo = new ErweimaInfo();
                        erweimaInfo.setRetcode("-1");
                        callbackeweima.callbackEweima(erweimaInfo);
                    }
                });
    }
