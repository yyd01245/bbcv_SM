...
import prod.nebula.dto.request.channelcenter.bgw.KeyDownRequest;
import prod.nebula.dto.response.channelcenter.bgw.KeyDownResponse;
		...
		if(hasParamErrors()){
			...
			try {
				if(!userInfoService.validUserToken(username, token)){//token是否失效
					...
					if(userinfo.getUser_status()==2||userinfo.getUser_status().equals(2)){//是否处于VOD状态
						mgwKeyControlService.keyControl(username, key_value,"2");
					}else if(userinfo.getUser_status()==3||userinfo.getUser_status().equals(3)){
						//键值发送到VGW
						mgwKeyControlService.keyControl(username, key_value,"3");
