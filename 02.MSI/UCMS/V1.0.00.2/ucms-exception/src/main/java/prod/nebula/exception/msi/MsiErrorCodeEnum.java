...
public enum MsiErrorCodeEnum implements ErrorCodeEnum {
	TOKEN_IS_ERROR(-2,"用户TOKEN错误!"),
	VOD_PLAY_FAIL(-3,"用户点播失败!"),
	USER_NOT_INVOD(-4,"请先登云或者点播VOD视频在进行按键操作!"),
	USER_UNBAND(-5,"用户未进行频道绑定或不在VOD状态下，不能进行视频播放!"),
	USER_BAND_FAIL(-6,"用户绑定失败!"),
	USER_CHOOSETIME_FAIL(-7,"用户选时播放失败!"),
	
	WEC_TOKEN_IS_ERROR(-2,"user token is error!"),
	WEC_VOD_PLAY_FAIL(-3,"user play vod false!"),
	WEC_USER_NOT_INVOD(-4,"please login frist or play vod first!"),
	WEC_USER_UNBAND(-5,"user unbinded,cannot play vod,please bind first"),
	WEC_USER_BAND_FAIL(-6,"user bind false!"),
	WEC_USER_CHOOSETIME_FAIL(-7,"user choosetime play false!"),
