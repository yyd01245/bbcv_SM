...
else if($dopost=="commendArchives")
{
    ...
    ShowMsg("成功把所选的内容设为推荐！",$ENV_GOBACK_URL);
    exit();
}
...
else if($dopost=="delArchives")
{
    ...
    {
        $wintitle = "内容管理-删除内容";
        $wecome_info = "<a href='".$ENV_GOBACK_URL."'>内容管理</a>::删除内容";
        $win = new OxWindow();
...
else if($dopost=='moveArchives')
{
    ...
    if(empty($totype))
    {
        ...
        echo "    <div class='titLeft'>移动内容</div>\r\n";
...
<?php
    ...
    else
    {
        ...
        ShowMsg("成功移动 $j 个内容！", $ENV_GOBACK_URL);
        exit();
    }
}
/*-----------------------------
...
else if($dopost=='return')
{
    ...
    ShowMsg("成功还原指定的内容！","recycling.php");
    exit();
}
/*-----------------------------
//清空内容
...
else if($dopost=='clear')
{
    ...
    if($fmdo=='yes')
    {
        ...
        ShowMsg("成功删除指定的内容！","recycling.php");
        exit();
    }
    else
    {
        ...
        if(empty($num))
        {
            ShowMsg("对不起，未发现相关内容！","recycling.php");
            exit();
        }
        $wintitle = "内容管理-清空所有内容";
        $wecome_info = "<a href='recycling.php'>内容回收站</a>::清空所有内容";
        $win = new OxWindow();
        $win->Init("archives_do.php","js/blank.js","POST");
        $win->AddHidden("fmdo","yes");
        $win->AddHidden("dopost",$dopost);
        ...
        $win->AddTitle("本次操作将清空回收站<font color='#FF0000'>所有共 $num 篇内容</font><br>你确实要永久删除“ $qstr ”这些内容？");
        $winform = $win->GetWindow("ok");
        $win->Display();
    }
}
...
else if($dopost=='del')
{
    ...
    {
        $wintitle = "内容管理-删除内容";
        $wecome_info = "<a href='recycling.php'>内容管理</a>::删除内容";
        $win = new OxWindow();
        $win->Init("archives_do.php","js/blank.js","POST");
        $win->AddHidden("fmdo","yes");
        $win->AddHidden("dopost",$dopost);
        ...
        $win->AddHidden("aid",$aid);
        $win->AddHidden("recycle",$recycle);
        $win->AddTitle("你确实要永久删除“ $qstr 和 $aid ”这些内容？");
        $winform = $win->GetWindow("ok");
        $win->Display();
    }
}
...
else if($dopost=='quickEditSave')
{
    ...
    if(!TestPurview('a_Edit'))
    {
        ...
        {
            CheckCatalog($typeid, "对不起，你没有操作栏目 {$typeid} 的内容权限！");
        }
        else
        {
...
else if($dopost=="makekw")
{
    ...
    $sp = null;
    ShowMsg("成功分析指定内容的关键词！", $ENV_GOBACK_URL);
    exit();
}
/*--------------------------
//批量增加属性
...
else if($dopost=='attsAdd')
{
    ...
    ShowMsg("成功对选中内容增加指定的属性！",$ENV_GOBACK_URL);
    exit();
}
/*--------------------------
//批量删除属性
...
else if($dopost=='attsDel')
{
    ...
    ShowMsg("成功对选中内容删除指定的属性！", $ENV_GOBACK_URL);
    exit();
}
/*--------------------------
//获得批量属性处理的AJAX窗体
