...
else if($action=='make')
{
    ...
    if($step==1)
    {
        ...
        OptimizeData($dsql);
        ShowMsg("完成数据优化，现在开始更新内容！","makehtml_all.php?action=make&step=2&uptype=$uptype&mkvalue=$mkvalue");
        exit();
    }
    //更新内容
    /*-------------------
    ...
    if($step==3)
    {
        ...
        ShowMsg("完成更新所有内容，现在开始更新栏目页！","makehtml_all.php?action=make&step=4&uptype=$uptype&mkvalue=$mkvalue");
        exit();
    }
    //更新栏目
    /*-------------------
