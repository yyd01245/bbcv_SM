...
//	int iDstPort = cSwitchInfo.iDstPort;
	...
	if(m_vectSendAddr.size() > 0)
	{
		...
		fflush(stdout);
		LOG_INFO_FORMAT("INFO  - [SWT]: Use same SRC src %s:%d  dst %s:%d \n",m_cSwitchInfo.strSRCIP,m_cSwitchInfo.iSRCPort,
	...
	else
	{
		...
		LOG_INFO_FORMAT("INFO  - [SWT]: Switch src %s:%d  dst %s:%d \n",m_cSwitchInfo.strSRCIP,m_cSwitchInfo.iSRCPort,
						m_cSwitchInfo.strDstIP,m_cSwitchInfo.iDstPort);
		...
		if ( (sock = socket(AF_INET, SOCK_DGRAM, 0))  == -1) {
			...
			return Error_Switch_CreatSocket;
		} else
...
//		setsockopt(sock,SOL_SOCKET,SO_SNDBUF,&nRecvBuf,sizeof(socklen_t));
		...
		if(iFirstIPHead >= 224)
		{
		    ...
		    if(err < 0)
		    {
		        ...
		        return Error_Switch_SetMULTICAST;
		    }
		    ...
		    if (err < 0)
		    {
		        ...
		        return Error_Switch_IP_ADD_MEMBERSHIP;
		    }
...
//	send_addr.sin_addr.s_addr = inet_addr(this0->m_cSwitchInfo.strDstIP);
	...
	while(1)
	{
		...
		//增加I帧对齐
		//先查找I帧1,过滤ts头2.找到pes 3.找到编码类型4.找到I帧

		int iFilterLen = 0;
/*		if(this0->m_bNeedFindIFrame)
		{
			bool bHaveFindIFrame = false;
			int iIFrameLen = 0;
			//bHaveFindIFrame=this0->m_tsStreamPrase.ParseStreamInfo(UDP_buf,len);
			bHaveFindIFrame=this0->m_tsStreamPrase.Find_Stream_IFrame(UDP_buf,len,&iIFrameLen);

			if(bHaveFindIFrame)
			{
				this0->m_bNeedFindIFrame = false;
				iFilterLen = iIFrameLen;
				printf("find Ifram len %d \n",iFilterLen);
			}
		}

		if(this0->m_bNeedFindIFrame)
		{
			//printf("---not find I frame \n");
		//	fflush(stdout);
			continue;
		}
*/
			int bHaveFindIFrame = -1;
			int iIFrameLen = 0;
			//bHaveFindIFrame=this0->m_tsStreamPrase.ParseStreamInfo(UDP_buf,len);
			bHaveFindIFrame=this0->m_tsStreamPrase.Find_PCR_Index(UDP_buf,len,&iIFrameLen);
			if(bHaveFindIFrame ==0)
			{
				if(pcr_timebase < 0)
				{
					//clock_gettime(CLOCK_MONOTONIC, &timens1);
					//pcr_timebase = timens1.tv_sec*1000*1000*1000+timens1.tv_nsec;
					gettimeofday(&tv1, NULL);
					pcr_timebase = tv1.tv_sec*1000*1000 + tv1.tv_usec;
					pcr_value = 0;
				}
				else
				{
					//clock_gettime(CLOCK_MONOTONIC, &timens2);
					//pcr_value = (timens2.tv_sec-timens1.tv_sec)*1000*1000*1000+timens2.tv_nsec-timens1.tv_sec;
					gettimeofday(&tv2, NULL);
					pcr_value = (tv2.tv_sec- tv1.tv_sec)*1000*1000 + tv2.tv_usec-tv1.tv_usec;
				}
				//ns us
				pcr_value = pcr_value*27;
			//	pcr_value += 271512000;
				this0->m_tsStreamPrase.Set_PCR_Value(UDP_buf,iIFrameLen,pcr_value);
			}
	
		uint8_t *pUDP_Filterbuf = UDP_buf+iFilterLen;
		int iFilterbufLen = len - iFilterLen;
		...
		while(itfid != this0->m_vectSendAddr.end())
		{
			...
			do
			{
				...
				iret = sendto(bindSock,pUDP_Filterbuf+iTotal,iFilterbufLen-iTotal,0,(struct sockaddr*)send_addr,sizeof(struct sockaddr));
...
int Switch_Stream::SetCheckFlag(int iFlag)
{
...
int Switch_Stream::CheckOneSwitch(SwitchInfo *cSwitchInfo)
{
	...
	while(itfind != m_vectSendAddr.end())
	{
		printf("-----find one check switch  vecsize=%d \n",m_vectSendAddr.size());
		fflush(stdout);
		struct sockaddr_in *send_addr = *itfind;
		if(NULL == send_addr)
		{
		...
		if(m_vectSendAddr.size() == 0)
		{
			printf("-----send_addr vector is empty ERROR \n");
			fflush(stdout);
			break;
		}
		printf("-----switch info %d,%s  \n", ntohs(send_addr->sin_port),inet_ntoa(send_addr->sin_addr));
		fflush(stdout);
		int iDst = ntohs(send_addr->sin_port);
		char strDstIP[128]={0};
		strcpy(strDstIP,inet_ntoa(send_addr->sin_addr));
