...
import prod.nebula.vgw4sida.module.resctrl.dto.VODResCtrlReqBean;
...
import prod.nebula.vgw4sida.util.client.TcpClient;
...
//			}
			...
			case scale:
				// 1.快进状态下按快进
				if (ctrl.getPlayClient().getScale() > 0
						&& keyValue.equals(VODConst.KEYVALUE_FAST)) {
					if (ctrl.getPlayClient().getScale() < VODConst.SCALE_FAST_TIMES) {
						ctrl.getPlayClient().setScale(
								ctrl.getPlayClient().getScale() + 2);
						doScale(ctrl);
					} else {
						doResume(ctrl);
					}
				...
				if (ctrl.getPlayClient().getScale() < 0
					...
					if (ctrl.getPlayClient().getScale() > VODConst.SCALE_SLOW_TIMES) {
						ctrl.getPlayClient().setScale(
								ctrl.getPlayClient().getScale() - 2);
						doScale(ctrl);
					} else {
...
//		json.put("dvbinfo", ctrl.getAreaid() + ":0:0");
	...
	private void tearDownClient(Controller ctrl) {
		...
		ctrl.getPlayClient().shutdown.set(true);
		ctrl.getPlayClient().shutdown();
	}
...
//	}
	...
	private void doChooseTime(Controller ctrl) {
		ctrl.getPlayClient().doChooseTime(ctrl.getCurrentTime());
		ctrl.setStatus(Commons.Status.play);
	}
