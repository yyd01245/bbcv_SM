...
public class G4Utils {
	...
	public static void copyPropFromBean2BusistpUserSubscriber(Object pFromObj, BusistpUserSubscriber busistpUserSubscriber) {
		if (busistpUserSubscriber != null) {
			try {
				busistpUserSubscriber.putAll(BeanUtils.describe(pFromObj));
				// BeanUtils自动加入了一个Key为class的键值,故将其移除
				busistpUserSubscriber.remove("class");
			} catch (Exception e) {
				log.error("==开发人员请注意:==\n 将JavaBean属性值拷贝到Dto对象发生错误啦!" + "\n详细错误信息如下:");
				e.printStackTrace();
			}
