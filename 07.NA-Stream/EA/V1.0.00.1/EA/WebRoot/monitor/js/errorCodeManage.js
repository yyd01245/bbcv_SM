...
Ext.onReady(function() {	
	...
	var store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
					url : './platmentMenage.ered?reqCode=queryHitch12'
				}),
		// 数据读取器
	...
	var grid = new Ext.grid.EditorGridPanel({
			...
			title : '<span style="font-weight:bold;">错误码信息</span>',
			autoScroll : true,
			frame : true,
			store : store, // 数据存储
			stripeRows : true, // 斑马线
...
//							forceFit : true,
	...
	var firstFormPanel = new Ext.form.FormPanel({
		...
		items : [{
				fieldLabel : '错误码', // 标签
				name : 'error_code_id', // name:后台根据此name属性取值
	...
	function addHitch(){
		if (firstWindow.getComponent('firstFormPanel').form.isValid()) {
					firstWindow.getComponent('firstFormPanel').form.submit({
						url : './platmentMenage.ered?reqCode=saveHitch12',
						waitTitle : '提示',
						method : 'POST',
						waitMsg : '正在处理数据,请稍候...',
						success : function(form, action) {
							...
							Ext.Msg.confirm('请确认', '错误码定义添加成功,您是否继续添加错误码定义?',
								function(btn, text) {
									if (btn == 'yes') {
										firstWindow.getComponent('firstFormPanel').form.reset();
									} else {
										firstWindow.hide();			
							}}
						)},
...
//			Ext.Msg.alert('提示', '请先选中要修改的用户');
	...
	function updateCodeItem() {
		...
		editCodeFormPanel.form.submit({
					url : './platmentMenage.ered?reqCode=updateErrorCode12',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
	...
	function deleteCodeItems() {
		...
		for (var i = 0; i < rows.length; i++) {
			if (rows[i].get('editmode') == '0') {
				fields = fields + rows[i].get('error_code_id') + '->' + '<br>';
			}
		}
		if (fields != '') {
		...
		var strChecked = jsArray2JsString(rows,'error_code_id');
		Ext.Msg.confirm('请确认', '你真的要删除错误码吗?', function(btn, text) {
				if (btn == 'yes') {
					Ext.Ajax.request({
							url : './platmentMenage.ered?reqCode=deleteHitch12',
...
//									},
	...
	var batchUpWindow = new Ext.Window({
		layout : 'fit',
		width : 300,
		height : 100,
		resizable : false,
		draggable : true,
		title : '<span>等级批量修改</span>',
		modal : true,
		collapsible : true,
		titleCollapse : true,
		maximizable : false,
	...
	function batchUpSave(){
		if (!batchForm.form.isValid()) {
			return;
		}
		var params = batchForm.form.getValues();
		var rows = grid.getSelectionModel().getSelections();
		var strChecked = jsArray2JsString(rows,'error_code_id');
		params.strChecked = strChecked ;
		batchForm.form.submit({
			url : './platmentMenage.ered?reqCode=batchErrorUpdate',
			success : function(response) {
				store.reload();
				batchUpWindow.hide();
				Ext.Msg.show({title:'提示',msg:'批量修改成功！',buttons:Ext.MessageBox.OK,icon:Ext.Msg.INFO});
			},
			failure : function(response) {
				var resultArray = Ext.util.JSON
						.decode(response.responseText);
	...
	function batchUpdate(){
		...
		if (Ext.isEmpty(record)) {
//			Ext.Msg.alert('提示', '请先选中要修改的用户');
		Ext.Msg.show({title:'提示',msg:'请先选中要修改的错误码项!',buttons:Ext.MessageBox.OK,icon:Ext.Msg.WARNING});
		return;
	}
		record = grid.getSelectionModel().getSelected();
		if (record.get('editmode') == '0') {
	...
	function exportModExcel(){
		exportExcel("js/errorCodeManage.xls") ; 
	}
		...
		buttons : [{
					...
					handler : function() {
						...
						formpanel.form.submit({
									url : './platmentMenage.ered?reqCode=importExcel12',
									waitTitle : '提示',
									method : 'POST',
									waitMsg : '正在处理数据,请稍候...',
									success : function(form, action) {
