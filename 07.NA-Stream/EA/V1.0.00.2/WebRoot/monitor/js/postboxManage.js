...
Ext.onReady(function() {								
	function queryData(){
		store.load({
			...
			header : '邮箱标识',
				dataIndex : 'postbox_id',
				sortable : true,
				hidden :true
			},{
				header : '用户名',
				dataIndex : 'postbox_username',
				sortable : true,
				width:230
			},{
				header : '邮箱地址',
				dataIndex : 'postbox_address',
				sortable : true,
				width:230
			},{
				header : '邮箱等级',
				dataIndex : 'postbox_level',
	...
	var store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
					url : './platmentMenage.ered?reqCode=queryPostbox'
				}),
		// 数据读取器
...
//							forceFit : true,
	...
	var firstFormPanel = new Ext.form.FormPanel({
		...
		items : [{
				fieldLabel : '用户名', // 标签
				name : 'postbox_username', // name:后台根据此name属性取值
				...
				fieldLabel : '邮箱地址', // 标签
				name : 'postbox_address', // name:后台根据此name属性取值
				allowBlank : false, // 是否允许为空
				anchor : '90%', // 宽度百分比
				vtype:"email",
				vtypeText:"不是有效的邮箱地址"
			},new Ext.form.ComboBox({
				hiddenName : 'postbox_level',
	...
	function addHitch(){
		if (firstWindow.getComponent('firstFormPanel').form.isValid()) {
					firstWindow.getComponent('firstFormPanel').form.submit({
						url : './platmentMenage.ered?reqCode=savePostbox',
						waitTitle : '提示',
						method : 'POST',
						waitMsg : '正在处理数据,请稍候...',
	...
	editCodeFormPanel = new Ext.form.FormPanel({
		...
		items : [{
				fieldLabel : '邮箱标识', // 标签
				name : 'postbox_id', // name:后台根据此name属性取值
				allowBlank : false, // 是否允许为空
				anchor : '90%',// 宽度百分比
				hideLabel: 'true', 
				hidden:'true'
...
//			Ext.Msg.alert('提示', '请先选中要修改的用户');
	...
	function updateCodeItem() {
		...
		editCodeFormPanel.form.submit({
					url : './platmentMenage.ered?reqCode=updatePostbox',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
	...
	function deleteCodeItems() {
		...
		for (var i = 0; i < rows.length; i++) {
			if (rows[i].get('editmode') == '0') {
				fields = fields + rows[i].get('postbox_id') + '->' + '<br>';
			}
		}
		if (fields != '') {
		...
		var strChecked = jsArray2JsString(rows,'postbox_id');
		Ext.Msg.confirm('请确认', '你真的要删除邮箱吗?', function(btn, text) {
				if (btn == 'yes') {
					Ext.Ajax.request({
							url : './platmentMenage.ered?reqCode=deletePostbox',
		...
		buttons : [{
					...
					handler : function() {
						...
						formpanel.form.submit({
									url : './platmentMenage.ered?reqCode=importPostboxExcel',
									waitTitle : '提示',
									method : 'POST',
									waitMsg : '正在处理数据,请稍候...',
									success : function(form, action) {
