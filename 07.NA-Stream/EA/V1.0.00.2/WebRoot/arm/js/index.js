...
Ext.onReady(function() {
		...
		Ext.Ajax.request({
					...
					success : function(response, opts) {
						...
						if (resultArray.flag == "1") {
							...
							Ext.Msg.show({title:'提示', msg:'密码错误,请重新输入!',fn:function() {
										lockForm.form.reset();
						...
						.confirm(
								...
								function(btn, text) {
									if (btn == 'yes') {
										...
										Ext.Msg.show({title:'提示',msg:	'请在任何时候按[F5]键刷新页面或者重新登录系统以启用['+ o.text + ']主题!',
											fn:function() {
													themeWindow.hide();
												},buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
					...
					setTimeout(function() {
								userFormPanel.form.load({
											...
											failure : function(form, action) {
												Ext.Msg.show({title:'提示',	msg:'数据读取失败:'+ action.failureType,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
											}
										});
							}, 5);
				});
	...
	function updateUser() {
		...
		userFormPanel.form.submit({
					...
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
						Ext.MessageBox.show({title:'提示', msg:'密码修改成功', fn:function() {
									userWindow.hide();
					...
					failure : function(form, action) {
						...
						if (flag == '0') {
							Ext.MessageBox.show({title:'提示',msg:'您输入的当前密码验证失败,请重新输入',fn:function() {
										Ext.getCmp('password2').setValue('');
										Ext.getCmp('password2').focus();
									},buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
						} else {
							Ext.Msg.show({title:'提示',	msg:'密码修改失败!',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
						}
					}
				});
	}
