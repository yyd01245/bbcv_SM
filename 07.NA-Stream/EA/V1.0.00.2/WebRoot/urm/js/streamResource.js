			...
			function singleFormSave() {
				...
				if (singleWindow.getComponent('singleForm').form.isValid()) {
						...
						failure : function(form, action) {
							...
							Ext.Msg.show({title:'提示',msg:'频道号记录已存在，请核对信息后再添加！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
...
//		});	
	...
	function updateCodeItem() {
		...
		editCodeFormPanel.form.submit({
					...
					failure : function(form, action) {
						Ext.Msg.show({title:'提示',msg:'流信息配置重复或重名，请核对信息！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
	/**
		...
		Ext.Msg.confirm('请确认', '你真的要删除此条信息吗?', function(btn, text) {
			if (btn == 'yes') {
				Ext.Ajax.request({
					...
					failure : function(response) {
						Ext.Msg.show({title:'提示',msg:'流信息删除失败！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					},
					params : {
						strChecked : checked
					}
