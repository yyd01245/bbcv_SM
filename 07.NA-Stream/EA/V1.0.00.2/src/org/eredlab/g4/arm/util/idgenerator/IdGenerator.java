...
public class IdGenerator {
     ...
     * @return
     */
	public static String getQamResIdGenerator(String pParentid){
		String maxSubQamResId = (String)g4Dao.queryForObject("IdGenerator.getMaxSubDeptId", pParentid);
		String qamResid = null;
		if(G4Utils.isEmpty(maxSubQamResId)){
			qamResid = "001";
		}else{
			int length = maxSubQamResId.length();
			String temp = maxSubQamResId.substring(length-3, length);
			int intQamResId = Integer.valueOf(temp).intValue() + 1;
			if(intQamResId > 0 && intQamResId < 10){
				qamResid = "00" + String.valueOf(intQamResId);
			}else if(10 <= intQamResId && intQamResId <= 99){
				qamResid = "0" + String.valueOf(intQamResId);
			}else if (100 <= intQamResId && intQamResId <= 999) {
				qamResid = String.valueOf(intQamResId);
			}else if(intQamResId >999){
				log.error(G4Constants.Exception_Head + "生成QAM资源编号越界了.同级兄弟节点编号为[001-999]\n请和您的系统管理员联系!");
			}else{
				log.error(G4Constants.Exception_Head + "生成QAM资源编号发生未知错误,请和开发人员联系!");
			}
		}
		return pParentid + qamResid;
	}
