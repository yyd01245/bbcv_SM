...
Ext.onReady(function() {
	...
	var grid = new Ext.grid.GridPanel({
				title : '<span style="font-weight:bold;">人员信息表</span>',
				iconCls : 'groupIcon',
				height : 500,
				// width:600,
				autoScroll : true,
	...
	grid.on("cellclick", function(grid, rowIndex, columnIndex, e) {
				...
				if (fieldName == 'userid' && columnIndex == 2) {
					...
					if (login_account == parent.DEFAULT_DEVELOP_ACCOUNT
							|| login_account == parent.DEFAULT_ADMIN_ACCOUNT) {
						if (usertype == '1') {
							Ext.MessageBox.show({title:'提示',
									msg:'超级管理员和开发人员不能对业务经办员授权<br>'
	...
	function addInit() {
		...
		addUserWindow.show();
		addUserWindow.setTitle('<span>新增人员</span>');
	...
	function saveUserItem() {
		...
		operpassword1 = Ext.getCmp('operpassword1').getValue();
		operpassword = Ext.getCmp('operpassword').getValue();
		if (operpassword1 != operpassword) {
			Ext.Msg.show({title:'提示', msg:'两次输入的操作密码不匹配,请重新输入!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			Ext.getCmp('operpassword').setValue('');
			Ext.getCmp('operpassword1').setValue('');
			return;
		}
		addUserFormPanel.form.submit({
					...
					failure : function(form, action) {
						...
						Ext.MessageBox.show({title:'提示',msg:'人员数据保存失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
	...
	function deleteUserItems() {
		...
		if (fields != '') {
			Ext.Msg.show({title:'提示',msg:'<b>您选中的项目中包含如下系统内置的只读项目</b><br>' + fields
							+ '<font color=red>系统内置人员不能删除!</font>',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		if (Ext.isEmpty(rows)) {
	...
	function editInit() {
		...
		if (Ext.isEmpty(record)) {
			Ext.MessageBox.show({title:'提示',msg:'请先选择要修改的项目!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		if (record.get('usertype') == '3') {
			Ext.MessageBox.show({title:'提示',msg:'系统内置人员,不能修改!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		addUserFormPanel.getForm().loadRecord(record);
		addUserWindow.show();
		addUserWindow.setTitle('<span>修改人员</span>');
	...
	function updateUserItem() {
		...
		operpassword1 = Ext.getCmp('operpassword1').getValue();
		operpassword = Ext.getCmp('operpassword').getValue();
		if ((password == '@@@@@@' && password1 == '@@@@@@')&&(operpassword == '@@@@@@' && operpassword1 == '@@@@@@')) {
			// Ext.getCmp('updatemode').setValue('1');
		} else {
			Ext.getCmp('updatemode').setValue('2');
			if (password1 != password) {
				Ext.Msg.show({title:'提示', msg:'两次输入的密码不匹配,请重新输入!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
				Ext.getCmp('password').setValue('');
				Ext.getCmp('password1').setValue('');
				return;
			}
	...
	function update() {
		addUserFormPanel.form.submit({
					...
					failure : function(form, action) {
						var msg = action.result.msg;
						Ext.MessageBox.show({title:'提示',msg:'人员数据修改失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
