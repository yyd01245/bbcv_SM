...
Ext.onReady(function() {	
	...
	Ext.Msg.maxWidth = 360;
			...
			var grid = new Ext.grid.GridPanel({
						...
						title : '<span style="font-weight:bold;">广告信息</span>',
						height : 300,
						autoScroll : true,
						frame : true,
						store : store, // 数据存储
			...
			function singleFormSave() {
				...
				if (singleWindow.getComponent('singleForm').form.isValid()) {
						...
						success : function(form, action) {
							refreshSessionTable();
							Ext.Msg.confirm('请确认', '广告添加成功,您是否继续添加新广告?',
									function(btn, text) {
										if (btn == 'yes') {
						...
						failure : function(form, action) {
							...
							Ext.Msg.show({title:'提示',msg:'该广告已存在或重名，请核对信息！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
...
//comboxWithTree,
{
	...
	function ininEditCodeWindow() {
		...
		if (Ext.isEmpty(record)) {
			Ext.Msg.show({title:'提示',msg:'请先选中要修改的广告!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
	...
	function updateCodeItem() {
		...
		editCodeFormPanel.form.submit({
					...
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
						Ext.Msg.show({title:'提示',msg:'广告信息更新成功！',buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
						editCodeWindow.hide();
						store.reload();
					},
					failure : function(form, action) {
						Ext.Msg.show({title:'提示',msg:'不同广告不允许推向相同的地址和端口，请重新修改数据！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
	/**
	...
	function deleteCodeItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.show({title:'提示',msg:'请先选中要删除的广告!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
			...
			if (btn == 'yes') {
				Ext.Ajax.request({
					...
					success : function(response) {
							Ext.Msg.show({title:'提示',msg:'广告删除成功！',buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
							store.reload();
					},
					failure : function(response) {
						Ext.Msg.show({title:'提示',msg:'广告删除失败！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					},
					params : {
						strChecked : checked
					}
