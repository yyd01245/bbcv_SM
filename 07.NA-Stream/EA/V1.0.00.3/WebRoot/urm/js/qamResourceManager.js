...
//				});	
			...
			function statusRender(value){
				if(value == 1)
					return "<span style= 'color:red;font-weight:bold;'>占用</span>";
				else if(value == 0){
					return "空闲";
				}
				else
			...
			if (btn == 'yes') {
				Ext.Ajax.request({
					...
					success : function(response) {
						var t= eval('('+response.responseText+')');
						  if(t.SUCCESS){
						   Ext.Msg.show({title:'提示',msg:'IPQAM资源删除成功,请确保该区域下导航流数目不大于现有资源数!',button:Ext.Msg.YES,icon:Ext.Msg.INFO});
						   store.reload();
						  }else{
						   Ext.Msg.show({title:'提示',msg:'IPQAM资源正在被用来点播，不能删除！',button:Ext.Msg.YES,icon:Ext.Msg.ERROR});
						  }
					},
					failure : function(response) {
						Ext.Msg.show({title:'提示',msg:'IPQAM资源正在被用来点播，不能删除！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					},params : {
