 ...
 * prior to every transfer requiring that the server connect to the tcpClient's
 ...
 * tcpClient are verified to ensure that they originated from the intended
 ...
 * close a connection if the tcpClient has been idle for longer than a
 ...
 * You may avoid server disconnections while the tcpClient is idle by
 ...
 * during periods of tcpClient inactivity, this can be alleviated by calling <pre>setReaderThread(false)</pre>.
...
implements Configurable
{
     ...
     * to occur between the tcpClient (local) and server and that the server
     * should connect to the tcpClient's data port to initiate a data transfer.
     ...
     * the tcpClient is connected to should connect to the other server's
     ...
     * A constant indicating the FTP session is expecting all transfers
     * to occur between the tcpClient (local) and server and that the server
     * is in passive mode, requiring the tcpClient to connect to the
     ...
     * A constant indicating the FTP session is expecting all transfers
     * to occur between two remote servers and that the server
     * the tcpClient is connected to is in passive mode, requiring the other
    ...
    throws IOException
    {
        ...
        return new org.eredlab.g4.ccl.net.io.SocketOutputStream(socket, output);
    }
     ...
     *      of the tcpClient being idle or some other reason causing the server
     ...
     * transfers to require the FTP server to connect to the tcpClient's
     ...
     * tcpClient to issue a PORT command before every data transfer.
     ***/
     ...
     * method only for data transfers between the tcpClient and server.
     ...
     * open a data port to which the tcpClient will connect to conduct
     ...
     * <code> _DATA_CONNECTION_MODE </code> prod.nebula.constants.
     * <p>
     ...
     * prod.nebula.constants, or if the type is <code> FTP.LOCAL_FILE_TYPE </code>, the
     ...
     *              <code>_FORMAT</code> prod.nebula.constants.  In the case of
     ...
     *         <code>_STRUCTURE</code> prod.nebula.constants).
     * @return True if successfully completed, false if not.
     * @exception FTPConnectionClosedException
     *      If the FTP server prematurely closes the connection as a result
     *      of the tcpClient being idle or some other reason causing the server
     ...
     *         <code>_TRANSFER_MODE</code> prod.nebula.constants).
     * @return True if successfully completed, false if not.
     * @exception FTPConnectionClosedException
     *      If the FTP server prematurely closes the connection as a result
     *      of the tcpClient being idle or some other reason causing the server
     ...
     * server to which the tcpClient is connected to retrieve a given file from
     * the other server.
     * <p>
    ...
    /***
     * Initiate a server to server file transfer.  This method tells the
     * server to which the tcpClient is connected to store a file on
    ...
    /***
     * Initiate a server to server file transfer.  This method tells the
     * server to which the tcpClient is connected to append to a given file on
    ...
    public InputStream retrieveFileStream(String remote) throws IOException
    {
        ...
        return new org.eredlab.g4.ccl.net.io.SocketInputStream(socket, input);
    }
     ...
     *                   as a result of the tcpClient being idle or some other
