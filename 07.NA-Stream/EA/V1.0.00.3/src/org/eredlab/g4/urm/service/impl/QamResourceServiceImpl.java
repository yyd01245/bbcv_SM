...
public class QamResourceServiceImpl extends BaseServiceImpl implements QamResourceService{
	...
	public Dto deleteQamResItems(Dto pDto) {
		...
		for (int i = 0; i < arrChecked.length; i++) {
			...
			List list = (List) urmDao.queryForList("QamResource.getResourceInfoByResID", dto);
			Dto resourceDto = (Dto)list.get(0);
			if(resourceDto.getAsInteger("state")==1){
				String stream_id = resourceDto.getAsString("stream_id");
				dto.put("iStreamID", stream_id);
				...
				if("E".equals(bindStatus)){
					...
					SendMessageClient s = new SendMessageClient(Contents.IPQAMRESOURCE,Contents.DELETE,bindUsername,stream_id);  
					s.send();
				}
			}else{
				//资源端口没有被使用，不做通知
...
//			if(res_status==1){
		...
		//		Dto delDto = (Dto)urmDao.queryForObject("QamResource.getQamResByKey",pDto) ;
			//	String rfCode = delDto.getAsString("rf_code");
				...
				SendMessageClient s = new SendMessageClient(Contents.IPQAMRESOURCE,Contents.UPDATE,null,String.valueOf(ipqamResId));  
				s.send();
