...
public class AdsServiceImpl extends BaseServiceImpl implements	AdsService {
	...
	private static Log log = LogFactory.getLog(AdsServiceImpl.class);
	...
	//	String id = pDto.getAsString("id");
		String advname = pDto.getAsString("advname");
		String advip = pDto.getAsString("advip");
		String advport = pDto.getAsString("advport");
		...
		//	String network_code = IDHelper.getnetworkID();
			...
			pDto.put("advname", advname);
			pDto.put("advip", advip);
			pDto.put("advport", advport);
			
			urmDao.insert("Ads.saveAds", pDto);	
			
			
			
			
		  	  SendMessageClient s = new SendMessageClient(Contents.AD,Contents.INSERT,null,"new");  
			  s.send();
...
//			urmDao.insert("Qam.saveOperLog",logDto);
	...
	public Dto deleteAds(Dto inDto) {
		Dto dto = new BaseDto();
...
//		Date create_date = new Date(new java.util.Date().getTime());
		for (int i = 0; i < arrChecked.length; i++) {
			...
			dto.put("id", id);
			  ...
			  SendMessageClient s = new SendMessageClient(Contents.AD,Contents.DELETE,null,id);  
			  s.send();
...
//			urmDao.insert("Network.saveNetworkHis",outDto);
			...
			//删除原表中的数据
			urmDao.delete("Ads.deleteAds", dto);
...
//		urmDao.insert("Qam.saveOperLog",logDto);
	...
	public Dto updateAds(Dto inDto) {
		...
		if(sum>0){
			...
			//	String id = pDto.getAsString("id");
			String advname = inDto.getAsString("advname");
			String advip = inDto.getAsString("advip");
			String advport = inDto.getAsString("advport");
			...
			//	String network_code = IDHelper.getnetworkID();
				//pDto.put("network_code", network_code);
			inDto.put("id", id);
			inDto.put("advport", advport);
			inDto.put("advip", advip);
			inDto.put("advname", advname);
		
			

			  SendMessageClient s = new SendMessageClient(Contents.AD,Contents.UPDATE,null,id);  
			  s.send();
//			Date create_date = new Date(new java.util.Date().getTime());
...
//			}
		...
		//	inDto.put("status",status);
			urmDao.update("Ads.updateAds", inDto);	
