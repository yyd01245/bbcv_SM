...
<script src="<%= base%>res/data/hdtvdata.js" ></script>
...
<%--打印日志--%>
 ...
 <%Logger logger = LoggerFactory.getLogger("【导航页日志】");logger.info("流化TV导航页面【streamid="+streamid+"】，【高清频道】"); %>
 ...
 function queryStatus(){    
     ...
     $.post("skipServlet?resolution=0&streamid="+steam_id+"&vodid="+vodid, function(data) {
...
var	timerule,movieList=[] ;
var id=getQueryStringByName("name"),currentEle,index=-1,timer,$star_area,tt,flag=0;
...
function changeMovie(moviename){
	...
	if(currentEle){
		...
		$("#qrcode").html("  <div  id='qrcode1'></div>");
	     $("#qrcode1").qrcode({width:340,height:340,text:"<%=basePath%>a?id=<%=streamid%>&vp="+currentEle.id+"&r=0"});	
