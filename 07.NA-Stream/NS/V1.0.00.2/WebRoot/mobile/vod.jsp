...
<script>
	...
	function initdata(){
		...
		nodesource.forEach(function(s){
			...
			newEle.find("a").attr("href","<%=basePath%>ShowVodInfo?flag=set&username=<%=username%>&streamid=<%=streamid%>&vodid="+s.id+"&name="+s.id+"&beforepage=vodjsp&bannername="+fclass.trim()+"&resolution="+<%=resolution%>);
			newEle.find("img").attr("src",s.mbPosterPath2);
