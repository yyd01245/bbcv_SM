...
<%--打印日志--%>
 ...
 <%Logger logger = LoggerFactory.getLogger("");
   logger.info("【导航页日志】流化TV导航页面【streamid="+streamid+"】，【标清频道】"); %>
 <script>
...
<script type="text/javascript" src="<%= base%>res/data/sdtvblurbgdata<%=streamid %>.js"></script>
...
<%--	$.get("http://218.108.50.246/qrcode", {channel:<%=streamid%>, vodid:1}, function(data){--%>
<%--		console.log(data);--%>
<%--		$("#qrcode1").attr("src", data.url);--%>
<%--		}, "jsonp")--%>
			//$("#qrcode1").attr("src", )
		 $("#qrcode1").qrcode({width:185,height:200,text:"<%=basePath%>a?id=<%=streamid%>&vp="+ids[index]+"&r=1"});	
        if(index==info.length){
        	moveCurrent(0);
        }
