...
public class PauseClient {
	...
	public void send(String id,String username){
		try {
			...
			os.write(s.getBytes());
			  logger.info("向SM服务器【"+serverIp+":"+serverPort+"】发送的报文:"+s);
		    ...
		    logger.info("UnknownHostException   PauseClient与 服务器 【 serverIp="+serverIp+" serverPort="+serverPort+" 】连接失败");
			
		} catch (IOException e) {
			   logger.info("IOException   PauseClient与 服务器 【serverIp="+serverIp+" serverPort="+serverPort+" 】连接失败");
		}
