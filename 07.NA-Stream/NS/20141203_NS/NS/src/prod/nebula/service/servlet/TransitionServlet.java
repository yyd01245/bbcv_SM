...
public class TransitionServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//			+ sb.toString().length());
			...
			if (value!=null&&!"".equals(value)) {
	           logger.info("【TransitionServlet】 2页面实时查找方法，因为value!=null， 页面将显示：‘即将播放’ ");
				application.removeAttribute(key);
				logger.info("【TransitionServlet】 application.removeAttribute(key) 已执行 ，所以页面不会再显示‘即将播放’");
				out.println("success");
				out.close();
			}else{
	...
	//		logger.info("清空文件内容");
			...
			logger.info("【TransitionServlet】3清除方法， 清除key，但没有清除成功，故页面还会出现‘即将播放’");
				
			} else {
				logger.info("【TransitionServlet】3清除方法， 清除key，清除成功，故页面不会出现‘即将播放’");
		...
		if ("1".equals(thisState)) {
			...
			logger.info("【TransitionServlet】用户点击播放按钮，播放影片《"+vodname+"》，rstp="+rstp+"");
			ServletContext application=this.getServletContext();   
	        //设置Application属性   		  
			...
			String key = username+"isplay";
			 ...
			 try {
				Thread.sleep(5000);
				application.removeAttribute(key);
				logger.info("【TransitionServlet】5秒后自动 清除key，清除成功，故页面不会出现‘即将播放’");
