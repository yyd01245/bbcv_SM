...
<script type="text/javascript" src="js/jquery.qrcode.min.js"></script>
 ...
 function queryStatus(){    
		...
		$.ajax({ url: "skipServlet?streamid=2", context: document.body, success: function(data){
        if(data!=""){
...
<div class="container">
	...
	<div class="left">
		<h1 id="movie">关云长</h1>
		<div class="video">
			<video id="video" src="video/gyc.mp4" autoplay="true" loop width="640" height="336"></video>
			<div class="info_bg">		
				<div class="movieinfo">
					<div class="infowin">
						<ul>
	...
	<div class="right">
		<div class="intro">
		   ...
		   <div style="height:440px;background-color: white;">
               <div  id="qrcode" style="position:relative; margin:0 auto; width:400px;top:2%; ">    
      </div>	
		</div>
	</div>
</div>
...
<script>
var id=getQueryStringByName("name"),currentEle,index=-1,timer,movieList=["gyc","aqgy","ypzjs"],$star_area;
var $descHeight, $winHeight,maxScroll,$videoBg,$win= $(".movieinfo"),$videoBg = $(".info_bg"),scrollTimer,direction=1;
$(function(){
	setClock();
	changeMovie(id);
	//$("#qrcode").qrcode({width:400,height:400,text:"http://192.168.100.11:8181/NS/a?id=1&vp=1"});		
	
	updataMaxScroll();
	
scrollTimer = setInterval(autoScroll,3000);
	
	timer=setInterval("changeMovie(checkIndex())",20000);
...
//alert(moviename);	
	...
	if(currentEle){
		...
		$("#qrcode").html("二维码："+currentEle.qrcode);
		$("#qrcode").qrcode({width:400,height:400,text:currentEle.qrcode});		
	}
