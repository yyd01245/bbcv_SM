...
<html>
  <head>
    ...
    <title>mb_detail_page.jsp</title>
...
<%--							<td class="table_1"><c:out value="${tv.useState}"/></td>--%>
							<td class="table_1" style="width:40%;">
								<input type="button" value="修改" onclick="location.href='apps/MbDetailServlet?method=preUpdateUrl&id=${tv.id}'">
								<c:if test="${tv.useState==1}"><input type="button" disabled="disabled" style="color: blue;" value="已用" ></c:if>
								<c:if test="${tv.useState==0}"><input type="button" value="使用" onclick="location.href='apps/MbDetailServlet?method=updateUseState&id=${tv.id}'"></c:if>

								<input type="button" value="下架" onclick="javascript:if(confirm('确认下架吗?'))window.location='apps/MbDetailServlet?method=updateState&id=${tv.id}'">
							</td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr style="font-size: 12px; font-family: '宋体'; height: 23px">
		...
		<td align="right" nowrap>
				...
				<a href="apps/MbDetailServlet?method=list&currentPage=1"><font title="首页">首页</font>&nbsp;</a>
				<a href="apps/MbDetailServlet?method=list&currentPage=${page.currentPage-1}"><font title="前一页"><c:if test="${page.currentPage-1>1 || page.currentPage-1==1}">前一页</c:if></font></a>
			
			
			<span style="color: #FF0000">[ ${page.currentPage} ]</span>
			
				<a href="apps/MbDetailServlet?method=list&currentPage=${page.currentPage+1}"><font title="下一页"><c:if test="${page.currentPage+1<page.totalPages || page.currentPage+1==page.totalPages }">下一页</c:if></font></a>&nbsp;
				<a href="apps/MbDetailServlet?method=list&currentPage=${page.totalPages}"><font title="最后一页">最后一页</font></a>
