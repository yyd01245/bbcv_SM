...
<%--							</td>--%>
							<td class="table_1"><c:out value="${tv.upData}"/></td>
							<td class="table_1"><c:out value=""/></td>
							<td class="table_1"><c:out value=""/></td>
<%--							--%>
<%--							 <c:if test="${tv.waitUsed=='0'}">--%>
<%--								<td class="table_1" >							--%>
<%--									<input type="button" value="否" onclick="location.href='apps/TvNavigateServlet?method=updateWait&id=${tv.id}'">--%>
<%--								</td>														--%>
<%--							</c:if>--%>
<%--							<c:if test="${tv.waitUsed=='1'}">--%>
<%--								<td class="table_1" >										--%>
<%--									<input type="button" style="color: red" value="是" onclick="location.href='apps/TvNavigateServlet?method=updateWait&id=${tv.id}'">																--%>
<%--								</td>														--%>
<%--							</c:if>	--%>
<%--							 <c:if test="${tv.pauseUsed=='0'}">--%>
<%--								<td class="table_1" >							--%>
<%--									<input type="button" value="否" onclick="location.href='apps/TvNavigateServlet?method=updatepause&id=${tv.id}'">--%>
<%--								</td>														--%>
<%--							</c:if>--%>
<%--							<c:if test="${tv.pauseUsed=='1'}">--%>
<%--								<td class="table_1" >										--%>
<%--									<input type="button" style="color: red" value="是" onclick="location.href='apps/TvNavigateServlet?method=updatepause&id=${tv.id}'">																--%>
<%--								</td>														--%>
<%--							</c:if>								--%>
							...
							<td class="table_1" style="width:40%;">
								<input type="button" value="修改" onclick="location.href='apps/TvNavigateConfigServlet?method=preUpdateAd&id=${tv.id}'">															
								<input type="button" value="下架" onclick="javascript:if(confirm('确认下架吗?'))window.location='apps/TvNavigateConfigServlet?method=changeState&id=${tv.id}'">
								
								
								
								
								<input type="button" value="下载" onclick="downLoadFile('${tv.adVideoUrl}');">	
							</td>
...
<script type="text/javascript">
	...
	function downLoadFile(fileName){  
	    window.location.href = "<%=basePath %>apps/ResUpDownLoad?flag=download&fileName="+encodeURIComponent(fileName);  
	}  
</script>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr style="font-size: 12px; font-family: '宋体'; height: 23px">
		...
		<td align="right" nowrap>
				...
				<a href="apps/TvNavigateConfigServlet?method=adList&currentPage=1"><font title="首页">首页</font>&nbsp;</a>
				<a href="apps/TvNavigateConfigServlet?method=adList&currentPage=${page.currentPage-1}"><font title="前一页"><c:if test="${page.currentPage-1>1 || page.currentPage-1==1}">前一页</c:if></font></a>
			
			
			<span style="color: #FF0000">[ ${page.currentPage} ]</span>
			
				<a href="apps/TvNavigateConfigServlet?method=adList&currentPage=${page.currentPage+1}"><font title="下一页"><c:if test="${page.currentPage+1<page.totalPages || page.currentPage+1==page.totalPages }">下一页</c:if></font></a>&nbsp;
				<a href="apps/TvNavigateConfigServlet?method=adList&currentPage=${page.totalPages}"><font title="最后一页">最后一页</font></a>
