  ...
  <body style="background: url(<%=basePath%>apps/images/1_right_1.jpg) repeat-x; padding: 3px 6px 0 6px;">
	...
	<table id="navTbl" width="100%" cellspacing="0" cellpadding="0"
		...
		<tr>
			...
			<td align="right">
				<a  href="apps/banner_add_update.jsp?flag=add" rel="a"><img src="<%=basePath %>apps/images/ico/8.gif" alt="新增"
						border="0">
				</a>
			...
			<TBODY id="detail">
				...
				<c:forEach items="${list}" var="tv" varStatus="tvn">
					<tr style="cursor: hand; text-align: center;"
							...
							<td class="table_1"><c:out value="${tv.itemNum}"/></td>
<%--							<td class="table_1"><c:out value="${tv.state}"/></td>--%>
<%--							<td class="table_1"><c:out value="${tv.useState}"/></td>--%>
							<td class="table_1" style="width:40%;">
								<input type="button" value="修改" onclick="location.href='apps/BannerServlet?method=preUpdateUrl&id=${tv.bannerId}'">
								
								
							
								<input type="button" value="下架" disabled="disabled" onclick="javascript:if(confirm('确认下架吗?'))window.location='apps/BannerServlet?method=updateState&id=${tv.bannerId}'">
							</td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr style="font-size: 12px; font-family: '宋体'; height: 23px">
		...
		<td align="right" nowrap>
				...
				<a href="apps/BannerServlet?method=list&currentPage=1"><font title="首页">首页</font>&nbsp;</a>
				<a href="apps/BannerServlet?method=list&currentPage=${page.currentPage-1}"><font title="前一页"><c:if test="${page.currentPage-1>1 || page.currentPage-1==1}">前一页</c:if></font></a>
			
			
			<span style="color: #FF0000">[ ${page.currentPage} ]</span>
			
				<a href="apps/BannerServlet?method=list&currentPage=${page.currentPage+1}"><font title="下一页"><c:if test="${page.currentPage+1<page.totalPages || page.currentPage+1==page.totalPages }">下一页</c:if></font></a>&nbsp;
				<a href="apps/BannerServlet?method=list&currentPage=${page.totalPages}"><font title="最后一页">最后一页</font></a>
