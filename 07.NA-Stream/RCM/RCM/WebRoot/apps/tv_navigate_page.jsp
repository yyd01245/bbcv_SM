...
<%--							<td class="table_1"><c:out value="${tv.useState}"/></td>--%>
							<td class="table_1" style="width:40%;">
								...
								<c:if test="${tv.useState==0}"><input type="button" value="使用" onclick="location.href='apps/TvNavigateServlet?method=updateUseState&id=${tv.id}'"></c:if>
								<input type="button" value="下架" <c:if test="${tv.useState==1}">disabled="disabled"</c:if> onclick="javascript:if(confirm('确认下架吗?'))window.location='apps/TvNavigateServlet?method=updateState&id=${tv.id}'">
							</td>
