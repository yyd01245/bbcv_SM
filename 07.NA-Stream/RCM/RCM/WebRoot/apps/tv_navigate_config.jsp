...
<table id="navTbl" width="100%" cellspacing="0" cellpadding="0"
		...
		<tr>
			...
			<td align="right">
				<a  href="apps/InitVodResourceServlet?page=tv_navigate_config_add_update.jsp&flag=add&resolution=${resolution}" rel="tv_navigate_config"><img src="<%=basePath %>apps/images/ico/8.gif" alt="新增"
						border="0">
				</a>
	...
	<!--明细   -->
	<form id="selectFormId" action="" name="selectForm">
		<TABLE width="99%" border="0" align="center" cellspacing="1"
			bgcolor="#BFC7CA" style="margin-top:0px;">
			<thead>
				<tr align="center">
...
<%--							<td class="table_1"><c:out value="模板URL"/></td>--%>
							<td class="table_1"><c:out value="${tv.vodId}"/></td>
							<td class="table_1"><c:out value="${tv.name}"/></td>
							<td class="table_1"><c:out value="${tv.pageDwellTime}"/></td>
							<td class="table_1"><c:out value="${tv.orderId}"/></td>
							
							<td class="table_1" style="width:40%;">
								<input type="button" value="修改" onclick="location.href='apps/TvNavigateConfigServlet?method=preUpdateUrl&id=${tv.id}&resolution=${resolution}'">
								

								<input type="button" value="下架" onclick="javascript:if(confirm('确认下架吗?'))window.location='apps/TvNavigateConfigServlet?method=updateState&resolution=${resolution}&id=${tv.id}'">
							</td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
	...
	<tr style="font-size: 12px; font-family: '宋体'; height: 23px">
		...
		<td align="right" nowrap>
				...
				<a href="apps/TvNavigateConfigServlet?method=list&currentPage=1&resolution=${resolution}"><font title="首页">首页</font>&nbsp;</a>
				<a href="apps/TvNavigateConfigServlet?method=list&currentPage=${page.currentPage-1}&resolution=${resolution}"><font title="前一页"><c:if test="${page.currentPage-1>1 || page.currentPage-1==1}">前一页</c:if></font></a>
			
			
			<span style="color: #FF0000">[ ${page.currentPage} ]</span>
			
				<a href="apps/TvNavigateConfigServlet?method=list&currentPage=${page.currentPage+1}&resolution=${resolution}"><font title="下一页"><c:if test="${page.currentPage+1<page.totalPages || page.currentPage+1==page.totalPages }">下一页</c:if></font></a>&nbsp;
				<a href="apps/TvNavigateConfigServlet?method=list&currentPage=${page.totalPages}&resolution=${resolution}"><font title="最后一页">最后一页</font></a>
