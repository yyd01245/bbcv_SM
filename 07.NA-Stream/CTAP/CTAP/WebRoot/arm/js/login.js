...
Ext.onReady(function() {
	...
	var addUserWindow = new Ext.Window({
				...
				draggable : false,
				closeAction : 'hide',
				title : '<span style="font-weight:bold;">注册新帐户</span>',
		...
		if (Ext.getCmp('loginForm').form.isValid()) {
			Ext.getCmp('loginForm').form.submit({
				...
				failure : function(form, action) {
					...
					Ext.Msg.show({title:'提示',msg: errmsg,fn: function() {
								var account = Ext.getCmp('loginForm')
										.findById('account');
	...
	function regAccount() {
		...
		if (values.password1 != values.password) {
			Ext.Msg.show({title:'提示',	msg:'两次输入的密码不匹配,请重新输入!',
								buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
		...
		addUserFormPanel.form.submit({
					...
					success : function(form, action) {
						addUserWindow.hide();
						Ext.Msg.show({title:'提示',	msg:'帐户注册成功,点击[登录]按钮进入系统!',buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
						var password = Ext.getCmp('loginForm')
								.findById('password');
						var account = Ext.getCmp('loginForm')
								.findById('account');
