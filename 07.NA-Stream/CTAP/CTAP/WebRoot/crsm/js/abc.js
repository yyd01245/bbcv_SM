...
//			root.appendChild(preOffLine);
			...
			for(var i=0;i<total;i++) {
				...
				if(groupJson.ROOT[i].vstatus=='normal'){//在运行
					offLined.appendChild(node[i]);
				}else if(groupJson.ROOT[i].vstatus=='register'){//未认证
					offLine.appendChild(node[i]);
				}else if(groupJson.ROOT[i].vstatus=='perunused'){//预下架
...
//					Ext.getCmp('updateConfig').disable();
			...
			//	store.proxy = new Ext.data.HttpProxy({url:'./crsmServerMonitor.ered?reqCode=querySesionDetail&server_ip='+nodeName});
				store.proxy = new Ext.data.HttpProxy({url:'./crsmServerMonitor.ered?reqCode=queryAds'});
...
//						store.reload();
		...
		function opRender(){
			...
			return '<a href="./ChartDetail.ered?reqCode=pageInit">图标详情</a>&nbsp;&nbsp;&nbsp'+'<a href="./Report.ered?reqCode=pageInit">报警管理</a>';
	...
	/**************E   右侧详情生成结束   E*****************/
			...
			var store = new Ext.data.Store({
				// 获取数据的方式
				proxy : new Ext.data.HttpProxy({
					url : './crsmServerMonitor.ered?reqCode=queryAds'
				}),
				// 数据读取器
				reader : new Ext.data.JsonReader({
