			 ...
			 for(var i=0;i<total;i++){
				(function(){
					...
					// var title='<div class="head"  >接入网关('+CabinetAddress+"-"+CabinetContainer+')</div>';//抬头/('+CabinetAddress+":600"+CabinetContainer+')
					  title='<div class="head" style="color:blue;font-weight:800;font-size:15px;" ><a href="#_" onclick="go()">接入网关<div class="head" style="display:inline;font-size:1px;font-weight:10;">('+cag_ip+":"+cag_port+')</div><div style="display:inline;font-size:1px;font-weight:100;"><br>版本号：VS-001,运行进程：Thread_NO1,<br>负载量：8000,运行状态：正常<br></div></a></div>';//抬头
...
//									alert(1+"."+j);
									...
									var kk= j;
								 var ckg_ip =groupRoot1[kk].ckg_ip;//
								 var ckg_port = groupRoot1[kk].ckg_port;//
								 
								 var cabinet_card = Math.ceil(Math.random()*3)
 
								 var ckg_id = groupRoot1[kk].ckg_id;//
						
								 var serverStatus = groupRoot1[kk].status;
								 var status = statusImg(groupRoot1[kk].status);
								 var type = changeType(groupRoot1[kk].status);
								 if(kk==0){
									 
									 cabinetDomm+='<div class ="each clearfix">键值网关:</div>'+'<div class ="each clearfix">'
									 ...
									 cabinetDomm+='<div class ="each clearfix">'
								...
								cabinetDom = cabinetDomm;
//								alert("ddd"+cabinetDom);
								}	
							}
						});	
//						alert("alert"+cabinetDom);
						 ...
						 Ext.Ajax.request({
							...
							//	alert(2);
									...
									for(var jj=0;jj<total1;jj++) {(function(){	 
//										alert(2+"."+jj);
										
										var kkk= jj;
									 var csg_ip =groupRoot1[kkk].csg_ip;//
									 var csg_port = groupRoot1[kkk].csg_port;//
									 
									 var cabinet_card = Math.ceil(Math.random()*3)
	 
									 var csg_id = groupRoot1[kkk].csg_id;//
							
									 var serverStatus = groupRoot1[kkk].status;
									 var status = statusImg(groupRoot1[kkk].status);
									 var type = changeType(groupRoot1[kkk].status);
									 if(kkk==0){
										 
										 cabinetDomm2+='<div class ="each clearfix">信令网关:</div>'+'<div class ="each clearfix">'
							    	 	   +'<a href="#_" onclick=editServer("'+csg_ip+'","'+serverStatus+'","'+csg_ip+'","'+csg_port+'")>'
							    	 	   +'<img src="'+status+'"/>'
							    	 	   +cabinet_card+',   '+csg_ip+':'+csg_port+'   '+type+'   '	    	 	
							    	 	   +'</a>'
							    	 	   +'</div>';//重写机柜模板 
									 }else{
										 
										 cabinetDomm2+='<div class ="each clearfix">'
								    	 	   +'<a href="#_" onclick=editServer("'+csg_ip+'","'+serverStatus+'","'+csg_ip+'","'+csg_id+'")>'
								    	 	   +'<img src="'+status+'"/>'
								    	 	   +cabinet_card+',   '+csg_ip+':'+csg_port+'   '+type+'   '					    	 	
								    	 	   +'</a>'
								    	 	   +'</div>';//重写机柜模板
