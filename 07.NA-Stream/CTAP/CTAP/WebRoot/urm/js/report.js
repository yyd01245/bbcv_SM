...
Ext.onReady(function() {	
	...
	Ext.Msg.maxWidth = 360;
			...
			function reportstateRender(value){
				if(value == 1){
					return "<span style= 'color:blue;font-weight:bold;'>接入网关</span>";
				}
				if(value == 2){
					return "<span style= 'color:blue;font-weight:bold;'>键值网关</span>";
				}
				if(value == 3){
					return "<span style= 'color:blue;font-weight:bold;'>信令网关</span>";
				}
				else{
			...
			function runstateRender(value){
				if(value == 1)
					return "<span style= 'color:red;font-weight:bold;'>一级</span>";
				else if(value == 2){
					return "<span style= 'color:red;font-weight:bold;'>二级</span>";
				}else if(value == 3){
					return "<span style= 'color:red;font-weight:bold;'>三级</span>";
				}
				else
					return "<span style= 'color:red;font-weight:bold;'>未知</span>";
			}
			...
			function opRender(){
				...
				return '<a href="./ChartDetail.ered?reqCode=pageInit">详情</a>&nbsp;&nbsp;&nbsp';
			...
			var store = new Ext.data.Store({
					...
					proxy : new Ext.data.HttpProxy({
								url : './Report.ered?reqCode=queryAds&advport='+reportType+'&loadnum='+gateway
							}),
					// 数据读取器
...
//comboxWithTree,
{
	...
	function startUseCodeItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.show({title:'提示',msg:'请先选中要启用的信息!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		var id = jsArray2JsString(rows,'id');
		//alert("strChecked="+strChecked);
		Ext.Msg.confirm('请确认', '你真的要启用此条信息吗?', function(btn, text) {
			if (btn == 'yes') {
				Ext.Msg.show({title:'提示',msg:'启用成功',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
//				Ext.Ajax.request({
