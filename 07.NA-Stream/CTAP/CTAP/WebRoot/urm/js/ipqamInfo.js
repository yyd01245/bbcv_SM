...
Ext.onReady(function() {
	...
	//运营区域下拉框
	var addAreaRoot  = new Ext.tree.AsyncTreeNode({
				id : '00',
				text : '中国',
				expanded : true
			});
		var addAreaDeptTree = new Ext.tree.TreePanel({
		...
		// 监听下拉树的节点单击事件
		addAreaDeptTree.on('click', function(node) {
					var nodeID=node.id;
					comboxAreaWithTree.setValue(node.text);
					comboxAreaWithTree.collapse();
			Ext.getCmp('deptid').setValue('001'+node.id);
				});
		 var comboxAreaWithTree = new Ext.form.ComboBox({
			name : 'deptname',
			...
			tpl : "<tpl for='.'><div style='height:390px'><div id='addAreaDeptTreeDiv'></div></div></tpl>",
			allowBlank : true,
			onSelect : Ext.emptyFn
		});
		// 监听下拉框的下拉展开事件
		comboxAreaWithTree.on('expand', function() {
				// 将UI树挂到treeDiv容器
				addAreaDeptTree.render('addAreaDeptTreeDiv');
				// addDeptTree.root.expand(); //只是第一次下拉会加载数据
				addAreaDeptTree.root.reload(); // 每次下拉都会加载数据
	...
	// 翻页排序时带上查询条件
		...
		var grid = new Ext.grid.GridPanel( {
			title : '<span style="font-weight:bold;">组织信息表</span>',
...
//			alert(node.attributes.id);
			if(node.attributes.id=='001'){
				...
				addAreaRoot.id=node.attributes.id.toString().substring(3,node.attributes.id.toString().length);
				addAreaRoot.text=node.attributes.text;
			}
...
//				anchor : '99%'
//			},
				comboxWithTree,comboxAreaWithTree,  {
				fieldLabel : '业务对照码',
				name : 'customid',
				allowBlank : true,
				anchor : '99%'
...
//				hidden : true
		...
		function saveDeptItem() {
			...
			addDeptFormPanel.form.submit( {
				...
				failure : function(form, action) {
					...
					Ext.Msg.show({title:'提示',	msg:'组织数据保存失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
				}
			});
		}
		...
		function editInit() {
			...
			if (Ext.isEmpty(record)) {
				Ext.Msg.show({title:'提示',	msg: '请先选择要修改的组织!',	buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
				return;
			}
			...
			addDeptWindow.show();
			addDeptWindow
					.setTitle('<span style="font-weight:bold;">修改组织</span>');
		...
		function update() {
			...
			addDeptFormPanel.form.submit( {
				...
				failure : function(form, action) {
					var msg = action.result.msg;
					Ext.Msg.show({title:'提示',	msg: '组织数据修改失败:<br>' + msg,buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
				}
			});
		}
		...
		function deleteDeptItems(pType, pDeptid) {
			...
			if (fields != '') {
				Ext.Msg.show({title:'提示',	msg:'<b>您选中的项目中包含如下系统内置的只读项目</b><br>' + fields + '<font color=red>只读项目不能删除!</font>',
								buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
				return;
			}
			if (Ext.isEmpty(rows)) {
				if (pType == '1') {
					Ext.Msg.show({title:'提示',	msg:'请先选中要删除的项目!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
					return;
				}
			}
			...
			Ext.Msg
					.confirm(
							'请确认',
							'<span style="color:red"><b>提示:</b>删除组织将同时删除组织下属人员和角色以及它们的权限信息,请慎重.</span><br>继续删除吗?',
							function(btn, text) {
								if (btn == 'yes') {
									if (runMode == '0') {
										Ext.Msg.show({title:'提示',	msg:'系统正处于演示模式下运行,您的操作被取消!该模式下只能进行查询操作!',
									...
									Ext.Ajax
											.request( {
												...
												success : function(response) {
													...
													Ext.Msg.show({title:'提示',	msg:resultArray.msg,buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
												},
