	...
	Ext.Msg.maxWidth = 360;
			...
			var store = new Ext.data.Store({
					...
					proxy : new Ext.data.HttpProxy({
								url : './SignalingLoadNumConfig.ered?reqCode=queryAds'
							}),
					// 数据读取器
			...
			var grid = new Ext.grid.GridPanel({
						...
						title : '<span style="font-weight:bold;">XXXXXXXXXXX</span>',
						height : 300,
						autoScroll : true,
						frame : true,
						store : store, // 数据存储
		...
		var singleForm = new Ext.form.FormPanel({
				...
				items : [{
					fieldLabel : '字段', // 标签
					name : 'field', // name:后台根据此name属性取值
					allowBlank : false, // 是否允许为空
					anchor : '100%', // 宽度百分比
					//maxLength:32,
					//maxLengthText : '广告名称的长度不能超过32'
				},{
					fieldLabel : '字段', // 标签
					name : 'field', // name:后台根据此name属性取值
					allowBlank : false, // 是否允许为空
			...
			function singleFormSave() {
				...
				if (singleWindow.getComponent('singleForm').form.isValid()) {
						...
						failure : function(form, action) {
							...
							Ext.Msg.show({title:'提示',msg:'该条信息已存在或重名，请核对信息！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
...
//comboxWithTree,
{
	...
	function ininEditCodeWindow() {
		...
		if (Ext.isEmpty(record)) {
			Ext.Msg.show({title:'提示',msg:'请先选中要修改的XXXXXXXX!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
	...
	function updateCodeItem() {
		...
		editCodeFormPanel.form.submit({
					...
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
						Ext.Msg.show({title:'提示',msg:'XXXXXXX信息更新成功！',buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
						editCodeWindow.hide();
						store.reload();
					},
					failure : function(form, action) {
						Ext.Msg.show({title:'提示',msg:'XXXXXXXXX配置重复或重名，请核对信息！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					}
				});
	}
	/**
			...
			if (btn == 'yes') {
				Ext.Ajax.request({
					...
					success : function(response) {
							Ext.Msg.show({title:'提示',msg:'XXXXXX删除成功！',buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
							store.reload();
					},
					failure : function(response) {
						Ext.Msg.show({title:'提示',msg:'XXXXXXXX删除失败！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					},
					params : {
						strChecked : checked
					}
	...
	function reviewCodeItems() {
		...
		if (Ext.isEmpty(rows)) {
			Ext.Msg.show({title:'提示',msg:'请先选中要审核的信息!',buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING});
			return;
		}
		var id = jsArray2JsString(rows,'id');
		//alert("strChecked="+strChecked);
		Ext.Msg.confirm('请确认', '确定要审核通过此条信息吗?', function(btn, text) {
			if (btn == 'yes') {
				Ext.Ajax.request({
					url : './adsManager.ered?reqCode=deleteAds',
					success : function(response) {
							Ext.Msg.show({title:'提示',msg:'XXXXXX审核成功！',buttons:Ext.Msg.OK,icon:Ext.Msg.INFO});
							store.reload();
					},
					failure : function(response) {
						Ext.Msg.show({title:'提示',msg:'XXXXXXXX审核失败！',buttons:Ext.Msg.OK,icon:Ext.Msg.ERROR});
					},
					params : {
						strChecked : checked
					}
