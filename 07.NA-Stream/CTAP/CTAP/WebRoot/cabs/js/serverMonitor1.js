...
//		   	  });
	 ...
	 RSMstroe = new Ext.data.Store({
						proxy : new Ext.data.HttpProxy({
									url: 'cabs/serverMonitor.ered?reqCode=queryServerStatusDetailList'
								}),
...
function openWindow(tstatus, id) {
	...
	var message = "IP为";
	Ext.Ajax.request({
     url: 'cabs/serverMonitor.ered?reqCode=queryServerDetail&vncm_id='+id,
     success: function(response){
     	current = "";
     	//启动，停止，升级,true为禁止
...
//     	}
		...
		var firstWindow = new Ext.Window({
			...
			buttons : [ {
				...
				handler : function() {
					Ext.Ajax.request({
					     url: 'cabs/serverMonitor.ered?reqCode=startServer&vncm_id='+id,
					     success: function(response){
								firstWindow.hide();
				...
				handler : function() {
					Ext.Ajax.request({
					     url: 'cabs/serverMonitor.ered?reqCode=stopServer&vncm_id='+id,
					     success: function(response){
								firstWindow.hide();
					     		var resultArray = Ext.util.JSON.decode(response.responseText);
								if(resultArray.success){
				...
				handler : function() {
					Ext.Ajax.request({
					     url: 'cabs/serverMonitor.ered?reqCode=updateServer&vncm_id='+id,
					     success: function(response){
								firstWindow.hide();
